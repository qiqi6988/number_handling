...
namespace {
  struct replace : public ModulePass {
    ...
    virtual bool runOnModule(Module &m) {
      ...
      for (Module::FunctionListType::iterator b = functions.begin(), be = functions.end(); b != be; ++b) {
        for (Function::BasicBlockListType::iterator i = b->begin(), ie = b->end(); i != ie; ++i) {
          for(BasicBlock::InstListType::iterator inst=i->begin(),inste=i->end();inst!=inste;inst++)
          {
