...
void copyRegs(int *registers, unsigned int i, unsigned int end,
	      FILE * otfil);

void
copyReg(int *registers, unsigned int i, unsigned int end, FILE * otfil)
{
  int n = 2;
...
void
copyRegs(int *registers, unsigned int i, unsigned int end, FILE * otfil)
{
  if (i < end) {
...
//      int *prebuf = buffers[0];
//      int *registers = buffers[1];
//      int *pstbuf = buffers[2];
...
#define pstbuf buffers[2]
  ...
  char *cmd = lookup(q, "cmd");

  if (!cmd) {
    fprintf(stderr, "Bad QUERY_STRING in environment\n");
    return 1;
  }

  argc = set_argv(argv + 1, cmd) + 1;
  ...
  while (holder < argc) {
    ...
    if (argv[holder][0] == '7') {
      ...
      end = getInteger(argv[holder + 2]);      //STONESOUP:CROSSOVER_POINT
...
//                              int *regs = registers;
//                              int value = *(registers + (int) (nn * nn - 4 + (3 - 2) * i));
//                              //print the contents
//                              printf("Register %d is: %d\n", i, value);
//                              //Copy everything into one string
//                              fprintf(otfil, "Register %d is: %d\n", i, value);
