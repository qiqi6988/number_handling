...
main(int argc, char **argv)
{
  ...
  assoc_t q = parse_query_string(query_string); //STONESOUP:DATA_TYPE:TYPEDEF
  if (!q) {
    fprintf(stderr, "cannot parse QUERY_STRING %s\n", query_string);
    return 1;
  }
  unsigned char lower = 0;
  unsigned char *ref = &lower;	//STONESOUP:DATA_FLOW:ADDRESS_ALIASING_1
  unsigned char upper = 0;
  for (; q->key; q++)
    if (!strcmp(q->key, "lower")) //STONESOUP:CONTROL_FLOW:CONDITIONAL
      *ref = (unsigned char)*q->val;
    else if (!strcmp(q->key, "upper"))
      upper = (unsigned char)*q->val;
  if (upper == 0 || lower == 0) {
