...
int main (int argc, char *argv[])
{
	...
	strncpy(filename, argv[1], 255);	//STONESOUP:SOURCE_TAINT:COMMAND_LINE	//STONESOUP:DATA_FLOW:ADDRESS_AS_A_CONSTANT
	filename[255] = '\0';

	// get max_line_length, which can be a char becuase it must be > 0 and <= 120
	max_line_length = (char) atoi ( argv[2] );	//STONESOUP:INTERACTION_POINT
	 ...
	 if ( place > max_line_length )
	   {
	     ...
	     if ( remaining_chars > 0 )	//STONESOUP:TRIGGER_POINT	//STONESOUP:CONTROL_FLOW:CONDITIONAL
	       {
	    	 output_data_index = 0;
	       }
